...
// @version      5.4
// @description  Sistema completo: Notificações + Logs + Atalhos personalizáveis + Detecção de URL
...
(function () {
  ...
  // Verifica se está na URL correta para exibir a interface
  function verificarURLValida() {
    return window.location.pathname.startsWith('/chat/');
  }

  // Controla a visibilidade da interface baseado na URL
  function atualizarVisibilidadeInterface() {
    ...
    if (urlValida) {
      // Estamos em /chat/* - mostrar interface
      floatingButton.style.display = 'flex';
      log('[URL] Interface ativada: área de chat detectada');

      // Reativar notificações se não estiverem rodando
      if (!notificationIntervalId) {
        startPeriodicNotification();
      }
    } else {
      // Estamos fora de /chat/* - ocultar interface
      ...
      // Pausar verificações de notificação (economiza recursos)
      if (notificationIntervalId) {
  ...
  // Monitora mudanças de URL e ajusta a interface
  function iniciarMonitoramentoURL() {
    ...
    // Estratégia 1: MutationObserver (detecta mudanças no DOM)
    ...
    // Estratégia 2: Polling (fallback confiável)
    setInterval(() => {
      if (urlAnterior !== window.location.pathname) {
        urlAnterior = window.location.pathname;
        atualizarVisibilidadeInterface();
    ...
    // Estratégia 3: Event Listeners (histórico do navegador)
  ...
  function runNotificationCheck() {
    // MODIFICADO: Verificar se está na URL correta antes de notificar
  ...
  function startPeriodicNotification() {
    // MODIFICADO: Verificar se está na URL correta antes de iniciar
    if (!verificarURLValida()) {
  ...
  function init() {
    ...
    // NOVO: Iniciar monitoramento de URL
    iniciarMonitoramentoURL();

    log('[NOTIFIER] Octadesk Notifier v5.4 inicializado');
    console.log('✅ Octadesk Notifier v5.4 carregado');
  }
